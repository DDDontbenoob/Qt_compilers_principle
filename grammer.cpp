...
Grammer::Grammer(ifstream& in){
  ...
  //打印first集合
  void Grammer::print_first(ofstream& ofs){
...
//打印follow集合
void Grammer::print_follow(ofstream& ofs){
  for(const string& X:vset){
      ofs<<"FOLLOW("<<X<<"):";
...
//打印所有产生式
void Grammer::print_allprod(){
  for(const auto& P:pvec)
    P.print();
}
//打印action表
void Grammer::print_actiontable(ofstream& os){
    print_table(os,get_printable_actiontable());
}
//获取可打印的action表
vector<vector<string>> Grammer::get_printable_actiontable(){
...
//打印表
void Grammer::print_table(ofstream&os,const vector<vector<string>>& table){
    int n=8;
...
//构造goto函数
Items Grammer::Goto(const Items& I,const string& X){
  Items J;
  string B;
...
//构建项集的闭包
Items Grammer::closure(const Items& I){
  Items J=I;
  int over=0;//是否结束
  string B;
...
unordered_set<string> Grammer::get_firstXs(const vector<string>& Xs){
  if(Xs.empty())return {};
